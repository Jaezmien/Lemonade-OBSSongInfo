...
song_info_lookup = list(song_info.keys())
def on_buffer_read(buffer):
	global song_info
	if buffer[0] == 0:
		i_file = open("info.txt", "w")
		i_file.write('')
		i_file.close()
	elif buffer[0] == 1:
		info_type = buffer[1]
		if info_type == 1:
			f_file = open("format.txt", "r")
			details = f_file.read()
			f_file.close()

			if '{{ SONG_TITLE }}' in details:
				details = details.replace('{{ SONG_TITLE }}', song_info['title'] or song_info['title_translit'])
			if '{{ SONG_SUBTITLE }}' in details:
				details = details.replace('{{ SONG_SUBTITLE }}', song_info['subtitle'] or song_info['subtitle_translit'])
			if '{{ ARTIST }}' in details:
				details = details.replace('{{ ARTIST }}', song_info['artist'] or song_info['artist_translit'])

			i_file = open("info.txt", "w")
			i_file.write(details)
			i_file.close()
		elif info_type > 1:
			song_info[ song_info_lookup[info_type - 2] ] = (decode_buffer(buffer[2:]) or "").strip()
